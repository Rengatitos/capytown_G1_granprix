...
DETALLE RETO 3.md):
    ...
    CHEQUEO_LADO (tambien fuera de la lista original) interrumpe
    AVANZAR_PARALELO cada ``distancia_chequeo_pared_m`` (bastante menos
    que una celda completa de 60cm) para verificar con distancia
    PUNTUAL si el lado seguido se abrio -- si se confirma (varios
    ciclos seguidos), se trata como haber llegado a la interseccion
    (mismo registro que el disparador normal de distancia/pared al
    frente, ver ``_avanzar_a_siguiente_fase``); si sigue ocupado,
    retoma AVANZAR_PARALELO. Evita depender solo de "avance 55cm, debo
    estar en un cruce" cuando la posicion por odometria no coincide
    exactamente con la geometria fisica real (causaba loops en celdas
    angostas, confirmado en pista).
...
class StateMachineNode(Node):
    ...
    def __init__(self):
        ...
        # Conteo de visitas por celda (col, row) -- usado SOLO para
        # romper loops/anillos cerrados en _elegir_direccion_sin_loop
        # (ver ese metodo), no es la memoria de visitadas completa que
        # se quito en una sesion anterior (esa hacia BFS de escape de
        # bolsillos). Confirmado en pista real: la regla de mano
        # izquierda pura queda dando vueltas indefinidamente en un
        # anillo cerrado (ciclo B2-B3-B4-C4-C3 repetido varias veces
        # sin avanzar) -- esto es un fallo matematico inevitable de
        # seguir-un-solo-lado en cualquier laberinto con un anillo, sin
        # importar la calibracion del LiDAR.
        self._celdas_visitadas = {(self._grid.col, self._grid.row): 1}
        ...
        self._plan_index = 0
        if self._ronda == 1:
            # Ronda 1 arma el mapa desde cero -- borra el archivo
            # guardado de una corrida anterior (si existe) para que el
            # dashboard no siga mostrando una ruta vieja como si fuera
            # la de esta corrida (confundia bastante: el mapa
            # descubierto se lee de disco sin importar de que corrida
            # es). Se vuelve a guardar igual que siempre al llegar a
            # META/TIMEOUT de ESTA corrida.
            self._borrar_mapa_anterior()
        elif self._ronda == 2 and self._usar_mapa_ronda2:
        ...
        # posicion desde donde se cuenta la distancia_chequeo_pared_m,
        # independiente de cell_start_xy (la celda completa de 60cm).
        self._chequeo_lado_start_xy = (0.0, 0.0)
        ...
        # Buzzer del robot: /beep, std_msgs/UInt16 con la duracion en ms
        # (confirmado que YB_Car_Node lo suscribe -- ver PROPIEDADES_ROBOT.md).
        # Adaptado de una rama mas madura del mismo proyecto.
    ...
    def _declare_parameters(self):
        defaults = {
            ...
            # Buzzer del robot -- ver nota junto a self._buzzer_pub.
            'buzzer_topic': '/beep',
            ...
            # 0.60), se detiene y verifica con distancia PUNTUAL si el
            # lado seguido se abrio -- en vez de solo confiar en que
            # "avance 55cm, debo estar en un cruce" (la posicion por
            # odometria puede no coincidir con la geometria fisica real
            # de la pista, sobre todo en celdas angostas). Valores
            # tomados de una rama mas madura del mismo proyecto
            # (logica_dos_reglas) que ya resolvio ahi el mismo loop en
            # espacios angostos con este mecanismo.
            'distancia_chequeo_pared_m': 0.12,
            'tiempo_chequeo_pared_s': 0.5,
            # "Vacio" (lado seguido abierto) tiene que sostenerse esta
            # cantidad de ciclos de control SEGUIDOS (a control_rate_hz,
            # no una sola lectura) antes de comprometerse a tratarlo
            # como una interseccion real -- un giro es una decision cara
            # de revertir. "Ocupado" no necesita esta confirmacion.
    ...
    def _handle_avanzar_paralelo(self):
        dx_chk = self._odom_x - self._chequeo_lado_start_xy[0]
        dy_chk = self._odom_y - self._chequeo_lado_start_xy[1]
        if math.hypot(dx_chk, dy_chk) >= self._distancia_chequeo_pared:
            # Chequeo periodico del lado seguido (cada
            # distancia_chequeo_pared_m, mucho mas frecuente que el
            # avance de celda completa de 60cm) -- ver _handle_chequeo_lado.
            self._publish_twist(Twist())
            self._contador_lado_libre = 0
    ...
    def _handle_chequeo_lado(self):
        """Chequeo periodico del lado seguido, cada
        ``distancia_chequeo_pared_m`` de avance (0.12-0.15m tipico, NO
        cada celda completa de 60cm) -- detenido primero
        (``tiempo_chequeo_pared_s``), y verifica con distancia PUNTUAL
        (no el ajuste de recta de wall_follower) si el lado seguido
        esta ocupado (pared) o vacio (abertura). Exige
        ``chequeo_pared_confirmaciones_ciclos`` lecturas SEGUIDAS de
        "vacio" (un giro es una decision cara de revertir) antes de
        tratarlo como haber llegado a la interseccion -- "ocupado" no
        necesita esa confirmacion, solo retoma el avance.

        Ataca la nota tecnica pendiente del proyecto: el punto de
        parada por distancia fija (0.55m) no siempre coincide con una
        abertura real, porque la posicion estimada por odometria puede
        no coincidir exactamente con la geometria fisica de la pista.
        En vez de asumir "avance 55cm, debo estar en un cruce", se
        verifica de verdad -- y bastante mas seguido -- que el lado se
        abrio, antes de comprometerse a girar. Idea tomada de una rama
        mas madura del mismo proyecto (``logica_dos_reglas``) que ya
        habia resuelto en pista el mismo loop en espacios angostos con
        este mecanismo (comentario textual encontrado ahi: "un giro
        ciego en un rincon angosto puede volver a encerrar al robot en
        el mismo bolsillo del que viene").
        """
        self._publish_twist(Twist())
        ...
        self._contador_lado_libre = 0
        # Ocupado: sigue habiendo pared del lado seguido -- retoma el
        # avance normal, reiniciando el contador de distancia del
        # chequeo periodico (NO el de la celda completa) desde aqui,
        # para no volver a dispararse de inmediato en el mismo lugar.
        self._chequeo_lado_start_xy = (self._odom_x, self._odom_y)
        self._set_state('AVANZAR_PARALELO')

    def _avanzar_a_siguiente_fase(self):
        """Registra la llegada a la siguiente celda (contador,
        GridTracker, mapa de visitadas/conectividad, evento) y decide
        el siguiente estado -- compartido entre el disparador normal
        de ``_handle_avanzar_paralelo`` (distancia de celda completa o
        pared al frente) y la confirmacion del chequeo periodico de
        ``_handle_chequeo_lado`` (lado seguido recien abierto)."""
        self._publish_twist(Twist())
        self._num_celdas += 1
    ...
    def _elegir_direccion_sin_loop(self, candidatos):
        """Entre las direcciones libres (``candidatos``, ya en orden de
        prioridad de mano), retorna la que lleve a la celda MENOS
        visitada -- en caso de empate (el caso normal, sin anillos
        cerrados) se preserva el orden de prioridad original, porque
        ``min()`` retorna el primer elemento con el valor minimo.

        Sin esto, la regla de mano pura queda dando vueltas
        indefinidamente si el laberinto tiene un anillo cerrado: se
        confirmo en pista real un ciclo de celdas (B2-B3-B4-C4-C3)
        repetido varias veces sin avanzar nunca hacia la meta -- un
        fallo matematico inevitable de seguir solo un lado en cualquier
        laberinto con un anillo, sin importar la calibracion del LiDAR.
        No es la memoria de visitadas completa que se quito en una
        sesion anterior (esa hacia BFS de escape de bolsillos sin
        salida); esta solo desempata hacia lo menos explorado.
        """
        if not candidatos:
            return None
        ...
        col, row, heading = self._grid.col, self._grid.row, self._grid.heading
        cols, rows = self._grid.num_columns, self._grid.num_rows
        heading_de_direccion = {
            'DERECHA': turn_right(heading),
            'NINGUNO': heading,
            'IZQUIERDA': turn_left(heading),
        }

        def visitas_de(direction):
            vecino = neighbor_cell(col, row, heading_de_direccion[direction], cols, rows)
            return self._celdas_visitadas.get(vecino, 0) if vecino is not None else 0

        return min(candidatos, key=visitas_de)

    def _borrar_mapa_anterior(self):
        """Borra el archivo de mapa guardado por una corrida anterior
        (si existe) al arrancar Ronda 1 -- ver nota junto al llamador
        en __init__. No es un error si el archivo no existe."""
        path = os.path.expanduser(self._mapa_path)
        try:
            os.remove(path)
            self.get_logger().info(f'mapa anterior borrado ({path}) -- Ronda 1 empieza desde cero')
        except FileNotFoundError:
            pass
        except OSError as exc:
            self.get_logger().warn(f'no se pudo borrar el mapa anterior ({path}): {exc}')
    ...
    def _handle_decidir(self):
        ...
        if direction is None:
            ...
            # se evalua SIEMPRE primero -- wall-following reactiva. Entre
            # las direcciones libres, se prefiere la que lleve a la
            # celda MENOS visitada (ver _elegir_direccion_sin_loop) --
            # en igualdad de visitas (el caso normal, sin loops) se
            # respeta este orden de prioridad tal cual, asi que el
            # comportamiento fuera de un anillo cerrado no cambia.
            if self._lado == 'IZQUIERDA':
                orden = (
                    ('IZQUIERDA', self._izquierda_libre),
                    ('NINGUNO', self._frente_libre),
    ...
    def _emitir_pitido_pare(self):
        ...
        duracion en ms a /beep. Patron adaptado de una rama mas madura
        del mismo proyecto (equivalente a
        ...
        pitido se dispara ya mismo; el resto queda en
        self._pitido_pendiente para que _despachar_pitido_pendiente los
        dispare en su momento segun tiempo transcurrido (nunca
        time.sleep, mismo estilo de "temporizador propio" del resto del
        nodo)."""
    ...
    def _despachar_pitido_pendiente(self, elapsed_desde_pare: float) -> None:
        """Dispara el resto del patron de _emitir_pitido_pare en su
        momento, segun el tiempo transcurrido desde que empezo el
        conteo de PARE (mismo elapsed que usa _handle_buscar_pare para
        el tiempo_pare_s)."""
        while self._pitido_pendiente and elapsed_desde_pare >= self._pitido_pendiente[0][0]:
